...
async function run() {
  ...
  console.log("Loading all articles from DB...");
  const allDbArticles = await Article.find({}).select("slug title status reviewStatus generationMode excerpt content").lean();
  console.log(`Loaded ${allDbArticles.length} total documents from DB.`);
  ...
  const appGetArticleVerified: any[] = [];
  const appGetArticleFailed: any[] = [];

  for (const item of articleUrls) {
    let doc = dbSlugMap.get(item.slug);
    let matchedSlug = item.slug;
    ...
    if (!doc && item.slug.includes("apos")) {
      ...
      if (doc) matchedSlug = normalizedSlug;
    }

    if (!doc) {
      missingUrls.push({ rawUrl: item.rawUrl, slug: item.slug });
    } else {
      const indexable = isArticleIndexable(doc as any);
      const issues = articleIndexabilityIssues(doc as any);

      // Also verify Next.js application function getArticleBySlug(matchedSlug)
      const appArticle = await getArticleBySlug(matchedSlug);
      if (appArticle) {
        appGetArticleVerified.push({ url: item.rawUrl, slug: matchedSlug });
      } else {
        appGetArticleFailed.push({ url: item.rawUrl, slug: matchedSlug, docStatus: doc.status, docReviewStatus: doc.reviewStatus, issues });
      }
  ...
  console.log("\n================ PASTED GSC LIST AUDIT SUMMARY ================");
  console.log(`Total URLs in Export: ${rawUrls.length}`);
  console.log(`Homepage URLs (HTTP 200 OK): ${homepageUrls.length}`);
  console.log(`Category Page URLs (HTTP 200 OK): ${categoryUrls.length}`);
  console.log(`Article URLs Analyzed: ${articleUrls.length}`);
  console.log(`  -> 🟢 Fully Valid Published & Indexable (HTTP 200 OK): ${validPublished.length}`);
  console.log(`  -> 🟢 Verified via getArticleBySlug(): ${appGetArticleVerified.length}`);
  ...
  fs.writeFileSync("scratch/audit-correct-results.json", JSON.stringify({
    stats: {
      total: rawUrls.length,
      homepage: homepageUrls.length,
      category: categoryUrls.length,
      validPublished: validPublished.length,
      appGetArticleVerified: appGetArticleVerified.length,
      appGetArticleFailed: appGetArticleFailed.length,
  ...
  console.log("Saved full report to scratch/audit-correct-results.json");
}
