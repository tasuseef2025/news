...
export function SectionHeader({ title, href }: { title: string; href?: string }) {
  return (
    <div className="mb-5 flex items-end justify-between gap-4 border-t-2 border-foreground pt-3">
      <h2 className="font-editorial text-2xl font-bold md:text-3xl">{title}</h2>
      {href ? (
      ...
      {articles.length ? (
        <div className="divide-y">
          {articles.map((article, index) => (
            <Link key={article.slug} href={`/news/${article.slug}`} className="grid grid-cols-[34px_1fr] gap-3 py-4">
              <span className="font-editorial text-2xl font-bold text-primary">{String(index + 1).padStart(2, "0")}</span>
              <span className="font-editorial text-[17px] font-bold leading-5 hover:text-primary">{article.title}</span>
            </Link>
          ))}
        </div>
      ) : (
...
export function CompactArticleList({ title, articles }: { title: string; articles: Article[] }) {
  return (
    <section className="border-t-2 border-foreground pt-4">
      <h2 className="font-editorial mb-3 border-b pb-3 text-2xl font-bold">{title}</h2>
      {articles.length ? (
        <div className="divide-y">
          {articles.map((article) => (
            <Link key={article.slug} href={`/news/${article.slug}`} className="grid grid-cols-[92px_1fr] gap-3 py-3">
              <ArticleImage src={article.image} alt={article.imageAlt || article.title} title={article.title} category={article.category} width={184} height={120} className="aspect-[4/3] object-cover" />
              <span className="font-editorial text-base font-bold leading-5 hover:text-primary">{article.title}</span>
            </Link>
          ))}
        </div>
      ) : (
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        {establishedCards.map((card) => (
          <Link
            ...
            className="group relative min-h-36 overflow-hidden border bg-card p-4"
          >
            {card.image ? (
            ...
            <div className="relative z-10 flex h-full flex-col justify-end">
              <h3 className="font-editorial text-xl font-bold group-hover:text-primary">{card.name}</h3>
...
export function AdvertisementSlot({
  ...
  if (!advertisement) {
    return (
      <a
        ...
        className={cn("group relative block overflow-hidden border bg-card", className)}
      >
  ...
  const body = (
    <div className={cn("relative min-h-28 overflow-hidden border bg-card p-5", className)}>
      {advertisement.image ? (
